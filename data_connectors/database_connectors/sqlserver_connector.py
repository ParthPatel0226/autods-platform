"""Microsoft SQL Server connector via SQLAlchemy + pyodbc."""
...
class SQLServerConnector(BaseConnector):
    ...
    def load(self, config: dict) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info("Executing SQL Server query (%.50s...)", query[:50])
            with engine.connect() as conn:
                df = pd.read_sql(text(query), conn)
            logger.info("Loaded %d rows x %d columns", len(df), len(df.columns))
            return df
        except ImportError:
            raise DataLoadError("Install pyodbc: pip install pyodbc")
        except Exception as e:
            raise DataLoadError(f"SQL Server load failed: {e}")

    def get_preview(self, config: dict, n_rows: int = 5) -> pd.DataFrame:
        query = config.get("query")
        if query:
            config = {**config, "query": f"SELECT TOP {n_rows} * FROM ({query}) sub"}
        elif config.get("table"):
            config = {
                **config,
                "query": f"SELECT TOP {n_rows} * FROM [{config['table']}]",
            }
